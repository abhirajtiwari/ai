...
def avoidObstacles(data):
    ...
    if np.amin(data[144:360]) < thresh and np.amin(data[695:720]) > thresh_side:
        move_command.linear.x = 0
        move_command.angular.z = -1
    elif np.amin(data[361:567]) < thresh and np.amin(data[0:25]) > thresh_side:
        move_command.linear.x = 0
        move_command.angular.z = 1
    else:
    ...
    if np.amin(data[0:25]) < thresh_side or np.amin(data[695:720]) < thresh_side:
