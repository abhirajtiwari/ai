	...
	for i in update:        
     #c= max(update, key = cv2.contourArea)
		...
		if j==0:
	     ...
	     if float(countw/tot)*100<50:
		continue 
	     		
	     roi=blur[y:y+yrange,x:x+xrange1]
	     
	     cv2.imshow('maskasas',roi)
		...
		m=h
	      ...
	      circles=cv2.HoughCircles(roi,cv.CV_HOUGH_GRADIENT,1,200,param1=200,param2=20,minRadius=int(w/3),maxRadius=int(w/2))
