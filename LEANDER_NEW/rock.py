...
q=GPIO.PWM(ledpin1,100)
r=GPIO.PWM(ledpin1,100)
s=GPIO.PWM(ledpin1,100)
...
def leftF(speedA):
    p.ChangeDutyCycle(speedA)
    print("leftF=", speedA)


def rightF(speedB):
    q.ChangeDutyCycle(speedB)
    print("rightF=", speedB)


def leftB(speedA):
    r.ChangeDutyCycle(speedA)
    print("leftB=", speedA)


def rightB(speedB):
    s.ChangeDutyCycle(speedB)
    print("rightB=", speedB)
...
while 1:
    for e in pygame.event.get():  # iterate over event stack
        if e.type == pygame.locals.JOYAXISMOTION:
            ...
            x = arduino_map(x, -1, 1, 0, 1023)
            y = arduino_map(y, 1, -1, 0, 1023)
            print("X=", x)
            print("Y=", y)
